class DecoratedHyperbolicTriangle {
    constructor(l1, l2, l3, standardize=true) {
        ...
        this.background_color = "#ffd5e9";
        this.color = "#ffa1ce";
        this.stroke = "#ff2f92";
        this.ray_color = "#9437ff";
        this.horocycle_color = "#ff7915";
        this.horocycle_fill_color = "#ffd30022";
        this.horocycle_width = 3;
        this.infinity_line = "#ff2f92";
    ...
    draw_halfspace_triangle(canvas) {
        canvas.ctx.fillStyle = this.background_color;
        ...
        canvas.ctx.beginPath();
        canvas.ctx.moveTo(offset_x + scale * halfspace_b.x, -5);
        canvas.ctx.lineTo(offset_x + scale * halfspace_b.x, offset_y);
        if (halfspace_b.x > halfspace_c.x) {
            canvas.ctx.arc(offset_x + scale * cx, offset_y, scale * r, 0, -Math.PI, true);
        } else {
            canvas.ctx.arc(offset_x + scale * cx, offset_y, scale * r, -Math.PI, 0, false);
        }
    ...
    draw_halfspace_horocycles(canvas) {
        ...
        canvas.ctx.fillStyle = this.horocycle_fill_color;
        canvas.ctx.lineWidth = this.horocycle_width;
        canvas.ctx.moveTo(-5, offset_y - scale * hp.h);
        canvas.ctx.lineTo(canvas.width+5, offset_y - scale * hp.h);
        canvas.ctx.lineTo(canvas.width+5, -5);
